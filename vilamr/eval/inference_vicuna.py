...
def random_list_split(full_list, ratio, shuffle=True):
    n_total = len(full_list)
    n1 = int(n_total * ratio)
    # n2 = int(n_total - n1)
    if shuffle:
        random.shuffle(full_list)

    return full_list[:n1], full_list[n1:]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_path", type=str,
        default="playground/checkpoints/vilamr-vicuna-13b",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--answers_file_id", type=str, default="xxx.jsonl"
    )
    parser.add_argument(
        "--for_debug", default=False, type=str2bool)
