...
from vilamr.train.llava_trainer import LLaVATrainer

from vilamr import conversation as conversation_lib
...
class ModelArguments:
    model_name_or_path: Optional[str] = field(
        ...
        default="playground/checkpoints/vicuna-13b-v1.5"
    )
...
def preprocess_llama3(
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, rou in enumerate(re_rounds):
            ...
            if has_image:
                round_len = len(
                    tokenizer_image_token(rou, tokenizer))  # + 1        # 整个问答的长度
                instruction_len = len(
...
def preprocess_mpt(
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for conv_idx in range(3, len(rounds), 2):
            re_rounds.append(
                conv.sep.join(rounds[conv_idx:conv_idx + 2]))  # user + gpt
        cur_len = 0
        target[:cur_len] = IGNORE_INDEX
        for i, rou in enumerate(re_rounds):
            if rou == "":
            ...
            if i != 0 and getattr(tokenizer, 'legacy',
                                  False) and IS_TOKENIZER_GREATER_THAN_0_14:
                round_len += 1
                instruction_len += 1
...
def preprocess_plain(
    ...
    for source in sources:
        ...
        conversation = source[0]['value'] + source[1][
            'value'] + conversation_lib.default_conversation.sep
...
def train(attn_implementation=None):
    ...
    if training_args.lora_enable:
        ...
        rank0_print(f"\033[33m{'=' * 120}\033[0m")
        rank0_print(model)
        rank0_print(f"\033[33m{'=' * 120}\033[0m")
    ...
    for name, param in model.named_parameters():
        ...
        if ("layers.0." in name) or ("stages.0.blocks.0." in name) or (
                "visual.0.blocks.0." in name) or ("projector" in name):
            if param.requires_grad:
                ptex = (f"\033[40;31m{name} is Trainable: {param.data.dtype}, "
                        f"{param.data.device}, {param.data.shape}\033[0m")
            else:
                ptex = f"\033[40;32m{name} is Frozen: {param.data.dtype}\033[0m"
            rank0_print(ptex)
    rank0_print(f"\033[91mTrainable param: {trainable_size / 1e6:.1f}M\033[0m")
    rank0_print(f"\033[91mTotal param: {total_size / 1e9:.1f}B\033[0m")
    rank0_print(f"\033[36m{'=' * 120}\033[0m")
    ...
    # train_dataset_2 = LazySupervisedDataset(
    #     tokenizer=tokenizer,
    #     data_path=f"playground/data/vimm/s2_mcot_alpha0.2.json",
    #     data_args=data_args
    # )
    # trainer.init_progressive_sft(train_dataset_2)
