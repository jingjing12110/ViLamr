...
def get_length_grouped_indices(
    ...
    megabatches = [split_to_even_chunks(megabatch, lengths, world_size)
                   for megabatch in megabatches]
...
class LengthGroupedSampler(Sampler):
    r"""
    Sampler that samples indices in a way that groups together features of
     the dataset of roughly the same length while
    ...
    def __iter__(self):
        if self.group_by_modality:
            ...
            indices = get_length_grouped_indices(
                self.lengths, self.batch_size, self.world_size,
                generator=self.generator)
        return iter(indices)
...
class LLaVATrainer(Trainer):
    ...
    def init_progressive_sft(self, data2):
        self.p2_dataset = data2
        self.p2_sft = True
        self.p2_dataset_length = len(self.p2_dataset)
        self.p1_dataset_length = len(self.train_dataset)
