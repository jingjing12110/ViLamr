...
from vilamr.constants import IMAGE_TOKEN_INDEX
...
def select_best_resolution(original_size, possible_resolutions):
    ...
    Args:
        original_size (tuple): The original size of the image in the
        format (width, height).
        possible_resolutions (list): A list of possible resolutions in the
...
def process_anyres_image(image, processor, grid_pinpoints):
    ...
    image_patches = [
        processor.preprocess(image_patch, return_tensors='pt')[
            'pixel_values'][0] for image_patch in image_patches
    ]
...
class KeywordsStoppingCriteria(StoppingCriteria):
    ...
    def call_for_batch(
            self, output_ids: torch.LongTensor,
